...
function submit (e){
    ...
    obtenerTipoSocio(tipoSocio);  // Asigna importe según tipo
    ...
    validarDocumento(documento);  // Valida largo > 3 y que no exista
    
    if (continuar != 'no') {
        nombre = document.getElementById('nombre').value;
        validarNombre(nombre);  // Valida largo > 3
    }
    
    // 3. Creación de cuotas
    if(continuar !== 'no') {
        // Crea una cuota por cada mes seleccionado
        for (i=1; i <= cantCuotas; i++) {
            idCuota = documento + '_' + i;
            const cuotaSocio1 = new CuotaSocio(
                nombre, 
                tipoDocumentoAux, 
                documento,
                socio,
            ...
            onClick: function() {
                Swal.fire({
                    ...
                    html: `
                        ...
                        <p><strong>Tipo de Socio:</strong> ${socio}</p>
                        <p><strong>Cuotas:</strong> ${cantCuotas}</p>
                    `,
                });
            }
        }).showToast();

        // Cerrar el modal después de crear el socio
    ...
    if (array.length === 0){
        ...
        array.forEach((item) => {
            if (item.cuotaPaga === true) {
                // Suma al total pagado
                totalPagado += item.importe;
        ...
        cuotasID.innerHTML += `
            <tr class="total-row total-pagado">
                <td colspan="4" class="text-end"><strong>Total Pagado:</strong></td>
                <td><strong>$${totalPagado}</strong></td>
                <td></td>
            </tr>
            <tr class="total-row total-pendiente">
                <td colspan="4" class="text-end"><strong>Total Pendiente:</strong></td>
                <td><strong>$${totalPendiente}</strong></td>
                <td></td>
            </tr>
        `;
    }
